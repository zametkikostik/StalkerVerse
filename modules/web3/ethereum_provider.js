/** Web3 Chromium Browser - EIP-1193 provider v2 */
(function () {
  'use strict';
  ...
  function sendToNative(method, params) {
    return new Promise((resolve, reject) => {
      ...
      if (window.__WEB3_NATIVE_BRIDGE__?.postMessage) {
        try { window.__WEB3_NATIVE_BRIDGE__.postMessage(JSON.stringify(message)); return; } catch (_) {}
      }
      if (window.Web3Bridge?.postMessage) {
        try { window.Web3Bridge.postMessage(JSON.stringify(message)); return; } catch (_) {}
      }
      try { window.postMessage(message, '*'); } catch (_) {}
      if (CONFIG.mockMode) {
        clearTimeout(timer); pending.delete(id);
        handleMock(method, params).then(resolve).catch(reject);
      }
    });
  }
  ...
  async function handleMock(method, params) {
    if (method === 'eth_requestAccounts' || method === 'eth_accounts') {
      if (!state.selectedAddress) {
        state.selectedAddress = CONFIG.mockAddress; state.isConnected = true;
        provider.selectedAddress = state.selectedAddress;
        emit('accountsChanged', [state.selectedAddress]); emit('connect', { chainId: state.chainId });
      }
    ...
    if (method.startsWith('personal_sign') || method.startsWith('eth_sign')) return '0x' + 'ab'.repeat(65);
